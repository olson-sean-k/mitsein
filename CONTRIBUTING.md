...
Mitsein targets stable Rust. See the [crate
manifest](https://github.com/olson-sean-k/mitsein/blob/master/Cargo.toml) for
the [MSRV]. However, a nightly toolchain is used for formatting (via
[`rustfmt`]) and memory safety analysis (via [Miri]). At a minimum, both a
stable and nightly toolchain are necessary to develop Mitsein, and can be
installed with the following [`rustup`] commands:
...
[`cargo-hack`] is recommended when changes affect conditional compilation
against feature flags, because it can be easy to break specific combinations of
interdependent features.

Commands for building, testing, etc. can be run via [`just`] with Mitsein's
[`justfile`](https://github.com/olson-sean-k/mitsein/blob/master/justfile).
These recipes reduce the tedium and error in common commands. For example, to
build documentation for all features with `docs.rs` feature annotations:
...
[`rstest`] for fixtures and cases. Many modules have a `harness` sub-module
where fixtures and other shared test code concerning that module are defined.
For example, the `vec1` module has the following sub-module structure for unit
tests:
...
Unit test functions feature descriptive names, typically with the form
