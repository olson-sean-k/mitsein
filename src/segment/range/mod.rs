...
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct UnorderedError<N>(N, N);
...
// NOTE: This should include adjacencies, though they are not strictly intersections.
...
// TODO: Is this trait even possible to implement for a range that is generic w.r.t. `N`?
pub trait IsStrictSubset<N> {
    fn is_strict_subset(&self, range: RangeInclusive<&N>) -> bool;
}

pub fn ordered_range_bounds<T, R>(range: R) -> Result<R, R>
where
    T: Ord,
    R: RangeBounds<T>,
{
